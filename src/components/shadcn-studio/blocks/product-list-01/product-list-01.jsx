...
const mapProductToUi = (product = {}) => {
  ...
  const hasDiscount = discount > 0 || Number(product.salePrice ?? 0) > 0
  const computedSalePrice = hasDiscount
    ? Number(product.salePrice ?? basePrice * (1 - discount / 100))
    : undefined
  ...
  return {
    ...
    images,
    imgAlt: product.imgAlt ?? product.title ?? 'Product image',
    href: product.href ?? (product.id ? `/product/${product.id}` : '/product'),
    badges,
    price: basePrice,
    salePrice: hasDiscount ? computedSalePrice : undefined,
    hasDiscount,
  }
}
...
const ProductList = ({
  ...
  const normalizedProducts = products?.map(mapProductToUi) ?? []
        ...
        <div className='grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3'>
          ...
          {normalizedProducts.map((product) => (
            <Card
              ...
              className={cn('border-none shadow-none', product.hasDiscount && 'relative')}>
              {/* Sale Badge */}
              {product.hasDiscount && (
                <Badge
                  className='bg-destructive/10 [a&]:hover:bg-destructive/5 focus-visible:ring-destructive/20 dark:focus-visible:ring-destructive/40 text-destructive absolute top-6 left-6 rounded-sm px-3 py-1 uppercase focus-visible:outline-none'>
                  Sale
                </Badge>
              )}
              ...
              <CardContent className='flex flex-1 flex-col justify-between gap-6'>
                {/* Product Image */}
                <a href={product.href}>
                  <img src={product.images[0]} alt={product.imgAlt} className='mx-auto size-50' />
                </a>
                  ...
                  <div className='flex flex-col gap-2 text-center'>
                    ...
                    <div className='flex items-center justify-center gap-2'>
                      {product?.badges?.map((badge) => (
                        <Badge
                          key={`${product.id ?? product.title}-${badge}`}
